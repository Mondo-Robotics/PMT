...
@configclass
class PaperTransformerObservationsCfg:
    """Observation groups for the paper-faithful RGMT deploy task.

    Actor groups intentionally exclude base linear velocity. The critic group
    keeps base velocity as a privileged input alongside reference height and
    tracked body-link poses.
    """

    # Paper (main.tex:353): the ACTOR receives NOISY observations while the CRITIC is
    # noise-free. We therefore enable corruption on the actor proprio/history/command
    # groups and keep the critic group noise-free below. (Set enable_corruption=False
    # at eval/deploy time via a *_PLAY override if a clean rollout is desired.)
    @configclass
    ...
    # Keep a minimal env-level policy group for generic tooling, but the runner
    # maps actor policy observations explicitly to proprio.
    policy = ProprioCfg()
    proprio = ProprioCfg()
...
@configclass
class RGMTG1EnvCfg(TrackingEnvCfg):
    """Flat-plane RGMT deploy env matching the paper observation/action design."""
    ...
    # Adaptive-sampling (hybrid) knobs. Active only when pmt_storage_mode == "hybrid".
    # All default to no-op so a plain hybrid run == streaming + bin_adaptive until tuned.
    ...
    observations = PaperTransformerObservationsCfg()
    ...
    def __post_init__(self):
        ...
        if self.pmt_storage_mode == "hybrid":
            # Adaptive-sampling command: memory-bounded streaming working set + the
            # hybrid bin sampler (Phases 0-4). Mirrors the CLEAN multi_motion_flat
            # streaming block (only fields that exist on MultiMotionCommandV2Cfg), NOT
            # the legacy streaming branch below (which references stale fields).
        ...
        self.commands.motion.update_action_offset_with_ref = True
        # Only the legacy eager MultiMotionCommandV2Cfg carries these fields. The hybrid
        # (AdaptiveSamplingMotionCommandCfg) and streaming cfgs manage storage themselves.
