...
def _get_input_dim(module: torch.nn.Module) -> int:
    """Walk into nested Sequential / MLP to find the first Linear layer's in_features."""
    if hasattr(module, "in_features"):
        return module.in_features
    for child in module.children():
        dim = _get_input_dim(child)
        if dim is not None:
            return dim
    return None
...
class _TorchPolicyExporter(torch.nn.Module):
    ...
    def __init__(self, policy, normalizer=None):
        ...
        elif hasattr(policy, "student_backbone") and policy.student_backbone is not None:
            # Multi-head StudentTeacher: compose backbone + action_head into a single Sequential
            self.actor = torch.nn.Sequential(
                copy.deepcopy(policy.student_backbone),
                copy.deepcopy(policy.student_action_head),
            )
        elif hasattr(policy, "student") and policy.student is not None:
...
class _OnnxPolicyExporter(torch.nn.Module):
    ...
    def export(self, path, filename):
        ...
        if self.is_recurrent:
            ...
            obs = torch.zeros(1, _get_input_dim(self.actor))
            torch.onnx.export(
                self,
                obs,
                os.path.join(path, filename),
