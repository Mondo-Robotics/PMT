...
from motion_tracking_rl.networks.layers import MLP, EmpiricalNormalization, HiddenState, Memory
...
class ActorCriticRecurrent(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        # Actor RNN (shared regardless of forward)
        ...
        if self.use_forward:
            ...
            # Actor MLP: rnn_hidden_dim → actor_hidden[:-1] → actor_hidden[-1]
            ...
            self.actor = MLP(rnn_hidden_dim, actor_output_dim, actor_hidden, activation)
    ...
    def _update_distribution(self, rnn_output: torch.Tensor, obs: TensorDict | None = None) -> None:
        """Update distribution given RNN output and optionally original observations.

        Args:
            rnn_output: Output from the actor RNN.
            obs: Original observations (required when using forward network).
        """
        if self.use_forward:
            # Actor outputs features
            features = self.actor(rnn_output)
            ...
            # Original: Actor directly outputs actions from RNN output
            if self.state_dependent_std:
                mean_and_std = self.actor(rnn_output)
                if self.noise_std_type == "scalar":
                    mean, std = torch.unbind(mean_and_std, dim=-2)
                elif self.noise_std_type == "log":
                    mean, log_std = torch.unbind(mean_and_std, dim=-2)
    ...
    def act(self, obs: TensorDict, masks: torch.Tensor | None = None, hidden_state: HiddenState = None) -> torch.Tensor:
        ...
        out_mem = self.memory_a(actor_obs, masks, hidden_state).squeeze(0)
        # Pass original obs for forward network to use
        self._update_distribution(out_mem, obs)
