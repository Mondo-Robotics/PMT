...
from motion_tracking_rl.networks import HiddenState
from motion_tracking_rl.utils import split_and_pad_trajectories
...
class RolloutStorage:
    class Transition:
        def __init__(self) -> None:
            ...
            self.cfm_info: dict[str, torch.Tensor] | None = None
    ...
    def fpo_mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        if self.training_type != "rl":
            raise ValueError("This function is only available for reinforcement learning training.")
        if not hasattr(self, "cfm_initial_loss"):
            raise ValueError("CFM storage is not initialized. Call FPOPlus.init_storage first.")
        ...
        # PPO fields (unused by FPO++ but kept for compatibility)
        old_actions_log_prob = self.actions_log_prob.flatten(0, 1)
        advantages = self.advantages.flatten(0, 1)
        old_mu = self.mu.flatten(0, 1)
        old_sigma = self.sigma.flatten(0, 1)
        ...
        # CFM fields
        cfm_initial_loss = self.cfm_initial_loss.flatten(0, 1)
        cfm_loss_eps = self.cfm_loss_eps.flatten(0, 1)
        cfm_loss_t = self.cfm_loss_t.flatten(0, 1)
        cfm_indices = indices.to(cfm_initial_loss.device) if cfm_initial_loss.device != indices.device else indices
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                stop = (i + 1) * mini_batch_size
                batch_idx = indices[start:stop]
                cfm_batch_idx = cfm_indices[start:stop]
                ...
                cfm_info_batch = (
                    cfm_initial_loss[cfm_batch_idx],
                    cfm_loss_eps[cfm_batch_idx],
                    cfm_loss_t[cfm_batch_idx],
                )
